...
const setupElementEvents = (letterElement) => {
	...
	letterElement.onmousedown = (event) => {
		const shiftX =
			event.clientX - letterElement.getBoundingClientRect().left
		...
		letterElement.onmouseup = () => {
			...
			const swappedElement = Array.from(letters).find((element) => {
				...
				return (
					...
					isInside(potentialRect, currentRect)
				)
			})
...
const isInside = (potentialRect, currentRect) => {
	return (
		(currentRect.right >= potentialRect.left &&
			currentRect.left <= potentialRect.left &&
			currentRect.bottom >= potentialRect.top &&
			currentRect.top <= potentialRect.top) ||
		(currentRect.right >= potentialRect.left &&
			currentRect.left <= potentialRect.left &&
			currentRect.top <= potentialRect.bottom &&
			currentRect.top >= potentialRect.top) ||
		(currentRect.left <= potentialRect.right &&
			currentRect.left >= potentialRect.left &&
			currentRect.bottom >= potentialRect.top &&
			currentRect.top <= potentialRect.top) ||
		(currentRect.left <= potentialRect.right &&
